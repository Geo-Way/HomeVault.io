...
async function loadCategoriesToUI() {
  ...
  store.getAll().onsuccess = (event) => {
    ...
    categories.forEach(cat => {
      if (!cat.parent && catSelect) {
        const opt = document.createElement("option");
        opt.value = cat.path; opt.innerText = cat.path;
        catSelect.appendChild(opt);
      }
      if (!cat.parent && modalParentSelect) {
  ...
  transaction.objectStore("categories").getAll().onsuccess = (event) => {
    event.target.result.forEach(cat => {
      if (cat.parent === selectedCat) {
        const subName = cat.path.split('/').pop();
        const opt = document.createElement("option");
...
async function syncToPC() {
  ...
  // 1. Enviar pendientes del móvil al PC
  const transaction = db.transaction(["documents"], "readwrite");
  ...
  // 2. Descargar archivos del PC al móvil (Lectura remota WebDAV)
  try {
    ...
    if (res.ok) {
      ...
      for (let i = 0; i < responses.length; i++) {
        ...
        if (!href || href.endsWith('/')) continue; // Omitir carpetas

        const urlParts = href.split('/Public/');
        if (urlParts.length < 2) continue;
        const relPath = decodeURIComponent(urlParts[1]);

        // Descargar el archivo del PC si no lo tenemos localmente
        ...
        if (fileRes.ok) {
          ...
          // Guardar en IndexedDB como sincronizado
          await new Promise((resolve) => {
...
async function createNewCategory() {
  ...
  const transaction = db.transaction(["categories"], "readwrite");
  transaction.objectStore("categories").put({ path: targetPath, parent: parent || (subCat ? mainCat : "") }).onsuccess = async () => {
    closeCategoryModal();
    document.getElementById("new-cat-input").value = "";
    document.getElementById("new-subcat-input").value = "";
    await loadCategoriesToUI();

    // Notificar al servidor del PC para crear la carpeta física por WebDAV
    try {
      const baseUrl = getBaseUrl();
      await fetch(`${baseUrl}/Public/${targetPath.split('/').map(encodeURIComponent).join('/')}`, { method: 'MKCOL' });
    } catch(e) { console.error("No se pudo crear en el PC remotamente", e); }
