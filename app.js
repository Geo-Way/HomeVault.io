...
// --- GESTIÓN DE INDEXEDDB (Base de datos local del móvil) ---
...
function getBaseUrl() {
  let savedIp = localStorage.getItem("homevault_pc_ip") || "localhost";
  return `http://${savedIp.replace(/^https?:\/\//, '').replace(/\/$/, '')}:8080`;
}

async function syncToPC() {
  const pcIp = prompt(currentLang === 'de' ? "Gib die IP-Adresse deines PCs ein:" : "Introduce la IP de tu PC:", localStorage.getItem("homevault_pc_ip") || "192.168.1.50");
  if (!pcIp) return;
