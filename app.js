...
const translations = {
  de: {
    ...
    ipLabel: "PC IP-Adresse (z.B. 192.168.1.50 oder localhost):",
  ...
  es: {
    ...
    ipLabel: "IP del PC (ej. 192.168.1.50 o localhost):",
...
function showView(viewId) {
  ...
  if (viewId === 'explore-view') {
    ...
    // Cargar la última IP guardada automáticamente en el campo de texto
    const savedIp = localStorage.getItem("homevault_pc_ip") || "192.168.1.50";
...
// --- SINCRONIZACIÓN CON EL PC (Memoria automática de IP) ---
function getBaseUrl() {
  const ipInput = document.getElementById("pc-ip-input");
  let pcIp = ipInput ? ipInput.value.trim() : "";
  if (!pcIp) {
    pcIp = localStorage.getItem("homevault_pc_ip") || "192.168.1.50";
  }
