...
request.onerror = (event) => {
    ...
    alert("Error accediendo a la base de datos local. Por favor, borra la caché del navegador.");
};
...
function abrirModalSeleccionOCreacion() {
    if (!db) {
        alert("La base de datos aún se está cargando. Espera un momento o recarga la página.");
        return;
    }
    const transaction = db.transaction(["docs"], "readonly");
    const store = transaction.objectStore("docs");
...
function guardarLocalmente() {
    ...
    if (!db) {
        alert("La base de datos aún no está lista. Por favor, recarga la página.");
        return;
    }
